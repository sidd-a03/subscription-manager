...
const SignIn = () => {
  ...
  const handleGoogleAuth = () => {
    // Navigate or link to Google OAuth flow
    const backendUrl =
      process.env.EXPO_PUBLIC_BACKEND_URL || 'http://10.0.2.2:8000'
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor }} className="bg-slate-50 dark:bg-slate-950">
      ...
      >
        ...
        >
          {/* Header */}
          <View className="mb-8 flex flex-col gap-1.5">
              ...
              render={({ field: { onChange, onBlur, value } }) => (
                <TextField isInvalid={Boolean(errors.password)}>
                  <View className="flex-row justify-between items-center mb-1">
                    ...
                    <TouchableOpacity
                      onPress={() => {
                        /* Action for forgot password if applicable */
                        Alert.alert('Forgot Password', 'Please use the web app to reset your password.')
                      }}
                    >
                      <Text className="text-sm text-teal-600 dark:text-teal-400 font-semibold">
                        Forgot password?
                      </Text>
                    </TouchableOpacity>
                  </View>
